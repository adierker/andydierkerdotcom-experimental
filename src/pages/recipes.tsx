...
import {RecipeListPageContent, RecipeListContent, RecipeListPageProps} from 'types'
import {convertContentToGetStaticPropsResult} from 'utils'
import {COLLECTIONS} from 'consts'

export const getStaticProps = async (): Promise<GetStaticPropsResult<RecipeListPageProps>> => {
  const recipeListPageContent = await getDocumentFromFirestore<RecipeListPageContent>(COLLECTIONS.PAGES, '/recipes')
  const recipeList = await getCollectionFromFirestore<RecipeListContent>(COLLECTIONS.RECIPES)
